...
- **ROI section** auto-renders when a client's sheet has an "ROI" tab. Schema: `Opportunity | Pipeline Value | Contract Value | Notes | <Mon YYYY> | <Mon YYYY> | ...`. One row per opportunity. The month-year columns start at the client's engagement month (earliest meeting in the Meetings tab) and run forward; clients add new month columns as time passes. **Past months that have values count as already-billed, future months as still-to-be-billed; Contract Value with no monthly split also counts as billed (fallback for unscheduled contracts).** Display: each PTG client dashboard renders a 5-column `RoiTable` (Opportunity / Pipeline / Total Contract / Billed / To Be Billed) — same-name rows on the sheet merge into one row on the dashboard. The PTG group dashboard renders a 3-column `GroupRoiTable` (Client / Revenue / Pipeline) with a footer totals row. Non-PTG dashboards (no ROI tab) hide both tables. Migration script: `scripts/seed-roi-tabs.js` (idempotent — deletes the existing tab, derives engagement start from each client's Meetings tab, and rebuilds with the in-script snapshot).
