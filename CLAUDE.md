...
- **Demo dashboard rolling dates** — when `CLIENT_NAME === 'Demo'`, all dates from the sheet are shifted on read so the latest meeting/lead lands on today. Keeps "this week" / "this month" filters populated forever without editing the sheet. Implemented in `shiftDatesToToday()` in `sheets-api.ts`; gated in the `/api/opportunities` route.
