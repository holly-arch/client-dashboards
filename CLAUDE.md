...
- **ROI section** auto-renders when a client's sheet has an "ROI" tab. Schema: `Month | Deal Name | Revenue | Pipeline | Notes` (deal-level — each row fills either Revenue or Pipeline). Dashboard sums each column for the headline figures and lists deal names in the revenueNote / pipelineNote ("BrynBuild £1,188.57 + Coffey Construction Ltd £9,107.14"). PTG group dashboard aggregates the same data across all member clients via the `/api/group` route — no hardcoded values anywhere. Migration script that seeded the initial values: `scripts/seed-roi-tabs.js`.
