...
- **ROI section** auto-renders when a client's sheet has an "ROI" tab. Schema: `Opportunity | Pipeline Value | Contract Value | Notes | <Mon YYYY> | <Mon YYYY> | ...`. One row per opportunity. The month-year columns start at the client's engagement month (earliest meeting in the Meetings tab) and run forward; clients add new month columns as time passes. Past months that have values count as already-billed, future months as still-to-be-billed (used by upcoming dashboard tiles — not yet surfaced). For the current ROICard headline the parser sums `Contract Value + all month columns` as revenue and reads `Pipeline Value` separately. Deal names appear in revenueNote / pipelineNote ("BrynBuild £1,188.57 + Coffey Construction Ltd £9,107.14"). PTG group dashboard aggregates across all member clients via `/api/group`. Migration script: `scripts/seed-roi-tabs.js` (idempotent — deletes the existing tab, derives engagement start from each client's Meetings tab, and rebuilds with the in-script snapshot).
