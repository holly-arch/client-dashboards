...
- **ROI section** auto-renders when a client's sheet has an "ROI" tab. Schema: `Opportunity | Pipeline Value | Contract Value | Notes | [Type of Service] | <Mon YYYY> | <Mon YYYY> | ...`. **One sheet row = one dashboard row** (no merging — same-name rows stay distinct, e.g. Trust Hire's YTL Revenue row + YTL Pipeline row appear as two separate entries). Each PTG client dashboard renders TWO side-by-side tables at the top: a `RevenueTable` (left, ~70% width — cols: Opportunity / Type of Service [if column exists] / Total Contract / Billed / To Be Billed) for rows with contract/monthly values, and a `PipelineDealsTable` (right, ~30% width — cols: Opportunity / Type of Service [if column exists] / Pipeline Value) for rows with pipeline values. Empty tables hide entirely. **Total Contract** reads from the Contract Value column directly (falls back to sum of monthly amounts for legacy rows). **Billed** = sum of monthly cells in past + current months only; **To Be Billed** = sum of future months only. Type of Service column is auto-detected (only Prime Secure's sheet has it currently). The PTG group dashboard renders a 3-column `GroupRoiTable` (Client / Revenue / Pipeline). Non-PTG dashboards (no ROI tab) hide everything. Migration script: `scripts/seed-roi-tabs.js`.
