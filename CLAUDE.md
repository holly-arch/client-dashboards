...
- **ROI section** auto-renders when a client's sheet has an "ROI" tab. Wide-format schema: `Deal Name | Type | Notes | <period 1> | <period 2> | ...`. One row per deal-and-type combination; period columns (e.g. "Historical", "May 2026", "Q2 2026") are auto-detected by header pattern (`isRoiPeriodHeader` in `sheets-api.ts`) so clients can add new month columns without us touching code. The dashboard sums every period column on every row to get the headline figure, and the `Type` column ("Revenue" or "Pipeline") routes each row to the right bucket. Deal names appear in revenueNote / pipelineNote ("BrynBuild £1,188.57 + Coffey Construction Ltd £9,107.14"). PTG group dashboard aggregates the same data across all member clients via the `/api/group` route — no hardcoded values anywhere. Migration script: `scripts/seed-roi-tabs.js` (idempotent — drops the existing tab and rebuilds it from the in-script snapshot).
