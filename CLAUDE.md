...
- **ROI section** auto-renders when a client's sheet has an "ROI" tab. Schema: `Opportunity | Pipeline Value | Annual Contract Value | Total Contract Value | Notes | [Type of Service] | <Mon YYYY> | <Mon YYYY> | ...`. **One sheet row = one dashboard row** (no merging — same-name rows stay distinct, e.g. Trust Hire's YTL Revenue row + YTL Pipeline row appear as two separate entries).
  - **Tabbed layout (PTG only).** Whenever `data.roi` is present, the dashboard renders a pill-style `DashboardTabs` next to the title with two tabs: **Campaign** (default) and **ROI**. The ROI tab houses everything ROI-related; the Campaign tab is the previous default content. Component: `src/components/DashboardTabs.tsx`.
  - **ROI tab content.** A `RoiTotalsCards` row of 4 KPI tiles at the top (Total 12-Month CV / Total Contract Value / Total Billed / Total Pipeline) followed by the `RevenueTable` (left, ~70% width — cols: Opportunity / Type of Service [if column exists] / Annual Contract / Total Contract / Billed / To Be Billed) + `PipelineDealsTable` (right, ~30% width — cols: Opportunity / Type of Service / Pipeline Value).
  - **Two contract columns on the sheet.** `Annual Contract Value` = the 12-month figure; `Total Contract Value` = the lifetime figure over the full contract duration. Legacy `Contract Value` header still parses (treated as annual) so unmigrated sheets stay functional — `Total Contract Value` then falls back to the annual figure until the new column is added.
  - **Per-opportunity `totalContract`** (used by RevenueTable's "Total Contract" column) prefers `totalContractValue`, falls back to `annualContractValue`, then to the sum of monthly amounts.
  - **Period-aware Billed / To Be Billed.** The time filter on the ROI tab restricts the monthly cells counted into Billed / To Be Billed (e.g. selecting Q3 2026 only counts Jul/Aug/Sep cells). Annual / Total / Pipeline tiles are time-independent. `all_time` preserves the original "everything past+current vs future" behaviour. Implemented via `monthInPeriod(year, month, period)` in [src/lib/utils.ts](src/lib/utils.ts).
  - **PTG group dashboard.** Same tabbed layout next to the GROUP OVERVIEW title. The ROI tab renders aggregate `RoiTotalsCards` (summed across all 6 clients) plus a wider `GroupRoiTable` (cols: Client / Annual Contract / Total Contract / Billed / To Be Billed / Pipeline / # Opps), each row pulling from that client's `data.roi.totals`. Sorted by Annual + Pipeline desc.
  - Non-PTG dashboards (no ROI tab) hide everything — `DashboardTabs` only renders when `data.roi` is present. Migration script: `scripts/seed-roi-tabs.js`.
...
- **Google Analytics section** (myBasePay only). Wires Composio's `GOOGLE_ANALYTICS_RUN_REPORT` action via `@composio/core` SDK and renders KPI tiles + top pages + top sources with a 7d/30d/90d range toggle. Gated on `CLIENT_NAME === 'myBasePay'` in `Dashboard.tsx`; the `/api/analytics` route returns 404 for any other dashboard. Requires env vars `COMPOSIO_API_KEY` (project-tier key from app.composio.dev) and `GA4_PROPERTY_ID` (numeric, e.g. `533917095`). The Composio user ID is hardcoded to `mybasepay` in `ga-api.ts` — must match what was typed in the Composio Connect Account dialog. Composio's manual `execute()` requires either a pinned toolkit version or `dangerouslySkipVersionCheck: true`; using the skip flag here is safe because the response shape we parse is Google's GA4 Data API (not Composio's wrapper), so the skip is genuinely safe. Files: `src/lib/ga-api.ts`, `src/app/api/analytics/route.ts`, `src/components/GoogleAnalyticsCard.tsx`. To add another client, grant the Composio user a new connection (User ID = `<client-slug>`), broaden the `ALLOWED_CLIENTS` set in the route and `COMPOSIO_USER_ID` lookup in `ga-api.ts`.
...
- **Demo dashboard rolling dates** — when `CLIENT_NAME === 'Demo'` OR `IS_DEMO === 'true'`, all dates from the sheet are shifted on read so the latest meeting/lead lands on today (at noon UTC, to avoid timezone-driven "tomorrow" rendering in BST etc.). Keeps "this week" / "this month" filters populated forever without editing the sheet. Implemented in `shiftDatesToToday()` in `sheets-api.ts`; gated in the `/api/opportunities` route. **The live demo project (`demo-dashboard` → demo.orrjodashboards.com) actually uses `CLIENT_NAME=Acme Corp`** (a fake brand we show prospects) plus `IS_DEMO=true` — never change the demo project's CLIENT_NAME to "Demo" or the header text will read "DEMO" instead of looking like a real client dashboard.
