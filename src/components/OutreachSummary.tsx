...
function MiniKpi({ label, value, color }: { label: string; value: number; color: string }) {
  return (
    <div className="rounded-lg p-4" style={{ background: '#141414', border: '1px solid #252525', borderTop: `3px solid ${color}` }}>
      <p className="text-xs font-semibold tracking-widest uppercase mb-1" style={{ color: '#9a9a9a' }}>{label}</p>
      <p className="text-2xl font-bold" style={{ color: '#fafafa' }}>{value.toLocaleString('en-GB')}</p>
    </div>
  );
}
    ...
    <div className="rounded-lg p-4 md:p-5" style={{ background: '#141414', border: '1px solid #252525' }}>
      <div className="mb-4">
        ...
        <div className="flex items-baseline gap-2">
          <span className="text-lg font-bold" style={{ color: '#fafafa' }}>Effort Behind the Meetings</span>
          <span className="text-sm" style={{ color: '#666' }}>{outreach.length} touches this period</span>
        </div>
      </div>
      ...
      <div className="hidden md:block overflow-x-auto">
        <table className="w-full text-sm">
          ...
          <tbody className="divide-subtle">
            {recent.map((o, i) => (
              <tr key={`${o.account}-${i}`} className="hover:bg-white/[0.03]">
                <td className="py-3 pr-3" style={{ color: '#888' }}>{formatDate(o.date) || '—'}</td>
                <td className="py-3 pr-3" style={{ color: '#b0b0b0' }}>{o.channel || '—'}</td>
                <td className="py-3 pr-3 font-medium" style={{ color: '#fafafa' }}>{o.account}</td>
                <td className="py-3" style={{ color: '#b0b0b0' }}>{o.outcome || '—'}</td>
              </tr>
            ))}
            {recent.length === 0 && (
              <tr><td colSpan={4} className="py-8 text-center" style={{ color: '#555' }}>Nothing logged yet for this period</td></tr>
            )}
          </tbody>
        </table>
      </div>
      ...
      <div className="md:hidden space-y-3">
        {recent.map((o, i) => (
          <div key={`${o.account}-${i}`} className="rounded-lg p-3" style={{ background: '#1a1a1a', border: '1px solid #252525' }}>
            <p className="font-medium text-sm" style={{ color: '#fafafa' }}>{o.account}</p>
            <p className="text-xs mt-1" style={{ color: '#888' }}>{o.channel} · {formatDate(o.date)}</p>
            <p className="text-xs mt-1" style={{ color: '#b0b0b0' }}>{o.outcome}</p>
          </div>
        ))}
        {recent.length === 0 && <p className="py-8 text-center text-sm" style={{ color: '#555' }}>Nothing logged yet for this period</p>}
      </div>
    </div>
  );
}
