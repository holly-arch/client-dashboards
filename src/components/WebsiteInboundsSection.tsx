...
const NEUTRAL = { bg: 'rgba(160,160,160,0.1)', fg: 'var(--color-text-secondary)', border: 'rgba(160,160,160,0.3)' };
...
function MiniKpi({ label, value, color }: MiniKpiProps) {
  return (
    <div
      ...
      style={{ background: 'var(--color-card)', border: '1px solid var(--color-border)', borderTop: `3px solid ${color}` }}
    >
          ...
          <tbody className="divide-subtle">
            {orderedInbounds.map((i) => (
              <tr key={i.id} className="row-hover align-top">
                ...
                <td className="py-3 pr-3 truncate max-w-[260px]" style={{ color: 'var(--color-text-secondary)' }}>
                ...
                <td className="py-3 max-w-[280px] whitespace-pre-wrap break-words" style={{ color: 'var(--color-text-muted)' }}>{i.notes}</td>
              </tr>
            ))}
            {inbounds.length === 0 && (
              <tr>
                <td colSpan={hasDate ? 7 : 6} className="py-8 text-center" style={{ color: 'var(--color-text-fainter)' }}>No website inbounds yet</td>
              </tr>
            )}
          </tbody>
        </table>
      ...
      <div className="md:hidden space-y-3">
        {orderedInbounds.map((i) => (
          <div key={i.id} className="rounded-lg p-3" style={{ background: 'var(--color-card-alt)', border: '1px solid var(--color-border)' }}>
            ...
            {i.notes && <p className="text-xs mt-2 whitespace-pre-wrap break-words" style={{ color: 'var(--color-text-muted)' }}>{i.notes}</p>}
          </div>
        ))}
        {inbounds.length === 0 && (
          <p className="py-8 text-center text-sm" style={{ color: 'var(--color-text-fainter)' }}>No website inbounds yet</p>
        )}
      </div>
    </div>
  );
