    ...
    <div className="rounded-lg p-4 md:p-5" style={{ background: '#141414', border: '1px solid #252525' }}>
      <div className="mb-4">
        ...
        <div className="flex items-baseline gap-2">
          <span className="text-lg font-bold" style={{ color: '#fafafa' }}>Engine Progress</span>
          <span className="text-sm" style={{ color: '#666' }}>{rows.length} metrics</span>
        </div>
      </div>
      <div className="grid grid-cols-1 md:grid-cols-3 gap-3 md:gap-4">
        {rows.map((m) => {
          ...
          return (
            <div key={m.metric} className="rounded-lg p-4" style={{ background: '#1a1a1a', border: '1px solid #252525' }}>
              <p className="text-xs font-semibold tracking-widest uppercase mb-2" style={{ color: '#9a9a9a' }}>{m.metric}</p>
              <div className="flex items-baseline gap-2">
                <p className="text-3xl font-bold" style={{ color: '#fafafa' }}>{fmt(m.value)}</p>
                {m.target && m.target > 0 && (
                  <p className="text-xs" style={{ color: '#888' }}>of {fmt(m.target)}</p>
                )}
              </div>
              {pct !== null && (
                <div className="mt-3 h-1 rounded-full overflow-hidden" style={{ background: '#252525' }}>
                  <div className="h-full" style={{ width: `${pct}%`, background: '#27ccd7' }} />
                </div>
              )}
              {m.note && <p className="text-xs mt-2" style={{ color: '#666' }}>{m.note}</p>}
            </div>
          );
        })}
      </div>
